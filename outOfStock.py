...
class Ui_outStock(object):
    def setupUi(self, Dialog):
        ...
        # intails
        colHeaders = ['Entry Date', 'Drug ID', 'Drug Name', 'Batch No.', 'Category', 'Quantity', 'Buying Price',
                      'Selling Price', 'Supplier', 'Reorder Level', 'Expiry Date', 'Added By', 'Created AT']
        ...
        self.refreshBTN.clicked.connect(self.loadOutStock)
        self.lineEdit.returnPressed.connect(lambda: self.searchOutStock("drugName"))
        self.lineEdit_2.returnPressed.connect(lambda: self.searchOutStock("batch"))
        self.medName_2.currentIndexChanged.connect(lambda: self.searchOutStock("category"))
        self.username.setText(str(PLC.pharmacyLoginController.getUsername()))

    #loadOutStock
    def loadOutStock(self):
        self.tableWidget.setRowCount(0)
        for row_number, row_data in enumerate(ADC.addDrugController.loadOutStock()):
            self.tableWidget.insertRow(row_number)
            for column_number, data in enumerate(row_data):
                self.tableWidget.setItem(row_number, column_number, QtWidgets.QTableWidgetItem(str(data)))

    #search OutStock
    def searchOutStock(self,searchType):
        eObj = E.errors()
        self.tableWidget.setRowCount(0)
        if searchType == "category":
            sword = self.medName_2.currentText()
            if self.medName_2.currentIndex() != 0:
                if len(ADC.addDrugController.searchOutStock(sword, searchType).fetchall()) > 0:
                    for row_number, row_data in enumerate(ADC.addDrugController.searchOutStock(sword, searchType)):
                        self.tableWidget.insertRow(row_number)
                        for column_number, data in enumerate(row_data):
                            self.tableWidget.setItem(row_number, column_number, QtWidgets.QTableWidgetItem(str(data)))
                else:
    ...
    def retranslateUi(self, Dialog):
        ...
        Dialog.setWindowTitle(_translate("Dialog", "Stock To Reorder | UG - Pharmacy Management System"))

        self.backImg.setText(_translate("Dialog", ""))
        self.title.setText(_translate("Dialog", "Stock To Reorder"))
